...
<script>
    ...
    function callUrl() {
        const url = "https://www.marketwatch.com/investing/stock/danske?countrycode=dk";
        $.get( url,
            function( data ) {
                priceToday(data);
                totalDiff();
    ...
    function totalDiff()
    {
        const theStartPrice = 30.58;
        $("#starterPrice").text(theStartPrice);
        let diff = theStartPrice - todayPrice;
        let percentage = (Math.abs(diff) / theStartPrice * 100).toFixed(2);
